                        ...
                        if (evtJson != null && !evtJson.isJsonNull()) {
                            ...
                            if (listeners != null) {
                                switch (evtJson.getAsString()) {
                                    case "READY":
                                        listeners.onReady(this, GSON.fromJson(data.get("user"), User.class));
                                    case "ERROR":
                                        listeners.onError(this, null, GSON.fromJson(data, ErrorEvent.class));
                                    case "ACTIVITY_JOIN":
                                        listeners.onActivityJoin(this, data.get("secret").getAsString());
                                    case "ACTIVITY_SPECTATE":
                                        listeners.onActivitySpectate(this, data.get("secret").getAsString());
                                    case "ACTIVITY_JOIN_REQUEST":
                                        listeners.onActivityJoinRequest(this, GSON.fromJson(data.get("user"), User.class));
                                }
                            }
                        }
                        // Disconnection
